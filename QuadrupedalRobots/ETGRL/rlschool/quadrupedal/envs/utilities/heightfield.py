...
class HeightField():
    def __init__(self,heightfieldSource):
        self.hf_id = 0
        self.terrainShape = 0
        self.heightfieldData = [0] * numHeightfieldRows * numHeightfieldColumns
        self.heightfieldSource = heightfieldSource

    def _generate_field(self, env, heightPerturbationRange=0.04):
        ...
        if self.heightfieldSource == useProgrammatic:
    ...
    def UpdateHeightField(self, heightPerturbationRange=0.04):
        if self.heightfieldSource == useProgrammatic:
            for j in range(int(numHeightfieldColumns / 2)):
                for i in range(int(numHeightfieldRows / 2)):
                    height = np.random.uniform(
                        0.0, 0.04)  # +math.sin(time.time())
                    self.heightfieldData[2 * i +
                                         2 * j * numHeightfieldRows] = height
                    self.heightfieldData[2 * i + 1 +
                                         2 * j * numHeightfieldRows] = height
            ...
            # terrain = p.pybullet_client.createMultiBody(0, terrainShape)
            # p.pybullet_client.resetBasePositionAndOrientation(
            #     terrain, [0, 0, 0.0], [0, 0, 0, 1])
            # p.pybullet_client.changeDynamics(terrain,
            #                                    -1,
            #                                    lateralFriction=FRICTION)
